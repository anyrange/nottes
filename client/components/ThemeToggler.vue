<template>
  <button class="theme-toggler default-border" aria-label="theme toggle" @click="setCurrentTheme">
    <span class="relative overflow-hidden w-5 h-5 flex items-center justify-center">
      <fa icon="desktop" class="theme-toggler-icon" :class="$colorMode.preference === 'system' ? 'show' : 'hide'" />
      <fa icon="moon" class="theme-toggler-icon" :class="$colorMode.preference === 'dark' ? 'show' : 'hide'" />
      <fa icon="sun" class="theme-toggler-icon" :class="$colorMode.preference === 'light' ? 'show' : 'hide'" />
    </span>
  </button>
</template>
