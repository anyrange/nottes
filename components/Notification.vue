<template>
  <div class="notify" :class="notifyClass">
    <div v-if="notification.progress && notification.delay" :style="progressStyle" class="notify-progress" />
    <div class="notify-box">
      <span class="notify-message">{{ notification.message }}</span>
      <span v-if="notification.closable" class="notify-close" @click="closeNotification(notification)">
...
export default {
  ...
  computed: {
    ...
    timeLeftPercent() {
      return Math.round((((this.timeLeft * 100) / this.notification.delay) * 100) / 100)
    },
    progressStyle() {
...
.notify-action-btn {
  @apply p-2 rounded-sm uppercase duration-100 hover:bg-white hover:bg-opacity-20;
}
</style>
